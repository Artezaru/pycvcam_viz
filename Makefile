...
latexpdf:
	...
	cd $(BUILDDIR)/latex && pdflatex pycvcam_viz.tex && pdflatex pycvcam_viz.tex
...
app:
	echo "from pycvcam_viz.__main__ import __main_gui__" > run_gui.py
	echo "__main_gui__()" >> run_gui.py
	pyinstaller --name pycvcam_viz --onefile --windowed run_gui.py
	rm run_gui.py
	rm -rf build
	rm run_gui.spec
